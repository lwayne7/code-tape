...
- 默认模型：`onnx-community/Qwen2.5-0.5B-Instruct`。
- 选择原因：该仓库标记为 Transformers.js / ONNX / text-generation，浏览器本地推理只拉取公开模型资产，不需要也不得携带 Hugging Face token；Qwen2.5 Instruct 对代码术语和结构化指令输出的通用能力优于更小的英文倾向模型，适合“前端术语纠错 + JSON + 章节生成”的后处理场景。
...
项目专属微调不作为默认路线。后续如确有大量已授权样本，可把 LoRA/PEFT 微调作为离线增强实验，训练、评估、量化和发布不在浏览器中完成，且必须先用 `npm run subtitle:dataset:validate` 和 `npm run subtitle:evaluate` 证明 JSON 合法率、稀疏 segment 引用合法率、非空章节监督率和术语保真率确有提升，再讨论是否替换默认模型。
