...
- 有音频的回放页可以提前 warm-up 本地 ASR 模型，点击生成时复用已加载 pipeline；warm-up 只下载/初始化模型，不提前转写音频。
...
- 加载策略：有音频的回放页可以先调用 `SubtitleTranscriber.warmUp()` 动态 import 和下载模型，降低首次点击“生成字幕”的等待；实际音频转写仍只在用户点击后发生。无音频或纯事件流回放不主动加载模型，warm-up 失败不阻断回放，点击生成时可以重试。
