...
- 识别语言策略：调用 Whisper 时使用 `task: "transcribe"`，但不强制单一 `language`；这样中文讲解、英文短句和中英混合片段都先尽量保留在原始 ASR 字幕中，再由后续 LLM 后处理负责简体中文一致性、术语纠错和英文保真。
...
2. 生成训练集：把“纠正专业术语、变量名、函数名、英文缩写、简体中文输出、中英混合保留、章节边界”整理成指令样本。用于微调的蒸馏样本必须包含至少一个章节，确保章节生成有监督信号；运行时模型无法可靠分段时仍可按上文返回空 `chapters` 并降级展示。
