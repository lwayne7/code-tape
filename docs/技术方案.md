...
项目专属微调作为默认后处理模型路线。后续如继续扩大已授权样本，可把 LoRA/PEFT 微调作为离线增强实验，训练、评估、量化和发布不在浏览器中完成，且必须先用 `npm run subtitle:dataset:validate` 和 `npm run subtitle:evaluate` 证明 JSON 合法率、稀疏 segment 引用合法率、非空章节监督率和术语保真率没有回退，再替换默认模型。
