...
- AI 字幕分两级：
  ...
  - 完整版：接入适合中文、前端代码术语和结构化 JSON 输出的本地 LLM，对 ASR 结果做术语/变量名纠错，并自动分段生成章节。
...
export type RecordingRepository = {
  ...
  // Optional cover asset generated from local video media. Missing thumbnails return null.
...
| `SubtitlePostProcessorWorkerClient` | 负责把本地 LLM 后处理请求转发到浏览器 Web Worker，避免生成阶段阻塞回放主线程 | 主线程发送 `warmUp/process/abort` 消息；worker 返回 JSON 纠错和章节结果 |
...
LLM 用于“ASR 后处理”，不替代 Whisper/ASR。默认不依赖项目专属微调模型，而是直接使用公开、Transformers.js 兼容、对代码术语和结构化 JSON 输出更稳的通用指令模型。它只消费已经带时间戳的字幕段和代码讲解上下文，prompt 只描述目标和输出格式，输出两个独立派生结果：

- **纠错后的字幕轨**：修正前端领域术语、组件名、变量名、函数名、包名、英文缩写和 code-tape 专有词；不在 prompt 中加入额外语言风格要求，优先保证本地小模型稳定输出可解析 JSON。
...
LLM 输出必须是可验证 JSON，用于更新字幕文本和生成章节。训练和运行时 prompt 侧的原始字幕字段命名为 `inputSegments`，assistant 输出字段才命名为 `segments`，避免本地小模型复制输入里的 `startMs/endMs/text` schema。为了降低浏览器本地生成耗时，`segments` 是稀疏变更集：只返回需要修改的字幕段，未返回的原始 ASR 字幕由应用层保留；`chapters` 仍必须总是返回数组，无法可靠分段时返回空数组。
...
- 默认模型：`ceilf6/code-tape-subtitle-postprocessor-onnx`。
- 选择原因：该公开 Hugging Face 仓库由项目字幕后处理蒸馏语料微调、合并并导出为 Transformers.js 可加载的 ONNX 资产，浏览器本地推理只拉取公开模型资产，不需要也不得携带 Hugging Face token；它的训练目标直接对齐“前端术语纠错 + JSON + 章节生成”的后处理场景。
- 加载策略：v12 默认只使用已通过本地 Transformers.js smoke test 的 WASM `q8` ONNX 资产，避免先探测未发布或不稳定的 `q4f16` / `q4` 资产造成额外等待。后续只有在 WebGPU 量化产物通过同等 smoke test 后，才能重新把 WebGPU 路径放回默认优先级；若最终仍失败，则保留原始 ASR 字幕并展示可恢复错误。
- 接入调用：本地可运行的公开模型走 `@huggingface/transformers` 浏览器本地推理，`VITE_SUBTITLE_POSTPROCESSOR_MODEL` 只用于本地调试或灰度覆盖，不能在浏览器 bundle 中携带 token。

项目专属微调作为默认后处理模型路线。后续如继续扩大已授权样本，可把 LoRA/PEFT 微调作为离线增强实验，训练、评估、量化和发布不在浏览器中完成，且必须先用 `npm run subtitle:dataset:validate`、`npm run subtitle:evaluate` 和 `npm run subtitle:postprocess:evaluate` 证明 JSON 合法率、稀疏 segment 引用合法率、非空章节监督率、术语保真率、章节时间线合法性和代表性 code-tape 前端讲解样例没有回退，再替换默认模型。
...
- `SubtitlePostProcessor`：构造本地 LLM prompt、解析严格 JSON、校验 `segments` 和 `chapters`、过滤未知或重复 segment id、对非法章节降级。
- `SubtitlePostProcessorWorkerClient`：验证默认后处理走 Web Worker、worker 成功响应能回传结果、请求取消会终止仍在运行的 worker。
...
- 微调语料契约：seed corpus 和 distilled SFT corpus 都至少 200 条，覆盖 React、TypeScript、Vite、Playwright、Vitest、Web Worker、WebGPU/WASM、IndexedDB、repo-guard、SubtitlePanel 和 chapters 等项目关键场景；语料不得包含 token 或 API key 形态文本。训练集必须以稀疏 `segments` 为主，质量门槛至少包含 `sparseOutputRate >= 0.85`、`fullSegmentOutputRate <= 0.15`、`averageOutputSegmentRatio <= 0.3`、`longTrackRecordRate >= 0.75`，避免小模型重新学成全量重写字幕而拉长浏览器生成时间。
