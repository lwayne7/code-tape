...
项目专属微调不作为默认路线。后续如确有大量已授权样本，可把 LoRA/PEFT 微调作为离线增强实验，训练、评估、量化和发布不在浏览器中完成，且必须先用 `npm run subtitle:dataset:validate` 和 `npm run subtitle:evaluate` 证明 JSON 合法率、稀疏 segment 覆盖、非空章节监督率和术语保真率确有提升，再讨论是否替换默认模型。
