...
LLM 输出必须是可验证 JSON，用于更新字幕文本和生成章节。为了降低浏览器本地生成耗时，`segments` 是稀疏变更集：只返回需要修改的字幕段，未返回的原始 ASR 字幕由应用层保留；`chapters` 仍必须总是返回数组，无法可靠分段时返回空数组。
...
- 加载策略：浏览器运行时优先尝试 WebGPU `q4f16`，再尝试 WebGPU `q4`，然后回退 WASM `q8` / `q4`。WebGPU 不可用或模型后端初始化失败时必须回退，不得阻塞回放；若最终仍失败，则保留原始 ASR 字幕并展示可恢复错误。
...
LLM 输出必须经过 schema 校验和安全兜底：JSON 解析失败、缺失 `segments` 或 `chapters`、出现未知 segment id、重复 segment id、空文本、时间戳越界、章节重叠或章节不按时间排序时，保留原始 ASR 字幕并记录 warning，不阻断播放；模型省略未改变的 segment 属于性能优化路径，不视为非法。如果只有章节非法而字幕纠错合法，可以保留纠错字幕并丢弃章节，或保留上一版合法章节，但不能污染字幕轨。
...
- 微调语料契约：seed corpus 和 distilled SFT corpus 都至少 30 条，覆盖 React、TypeScript、Vite、Playwright、Vitest、Web Worker、WebGPU/WASM、IndexedDB、repo-guard、SubtitlePanel 和 chapters 等项目关键场景；语料不得包含 token 或 API key 形态文本。
